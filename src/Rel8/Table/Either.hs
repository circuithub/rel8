...
instance
  (context ~ Expr, Table Expr a, Table Expr b) =>
...
instance
  ...
  type
    Transpose to (EitherTable context a b) =
      EitherTable to (Transpose to a) (Transpose to b)


  toColumns EitherTable{tag, left, right} =
    HEitherTable
  ...
  fromColumns HEitherTable{htag, hleft, hright} =
    EitherTable
...
instance
  (ToExprs exprs1 a, ToExprs exprs2 b, x ~ EitherTable Expr exprs1 exprs2) =>
