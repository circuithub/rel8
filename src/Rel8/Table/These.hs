...
import Rel8.Expr.Null (isNonNull, null)
...
instance
  (context ~ Expr, Table Expr a, Semigroup a) =>
...
instance
  (context ~ Expr, Table Expr a, Table Expr b, Semigroup a, Semigroup b) =>
...
instance
  ...
  type
    Transpose to (TheseTable context a b) =
      TheseTable to (Transpose to a) (Transpose to b)


  toColumns TheseTable{here, there} =
    HTheseTable
...
instance
  (EqTable a, EqTable b, context ~ Expr) =>
...
instance
  (OrdTable a, OrdTable b, context ~ Expr) =>
...
instance
  (ToExprs exprs1 a, ToExprs exprs2 b, x ~ TheseTable Expr exprs1 exprs2) =>
  ToExprs x (These a b)
