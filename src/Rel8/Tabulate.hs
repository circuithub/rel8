...
-- opaleye
import qualified Opaleye.Order as Opaleye (distinctOnExplicit, orderBy)
...
import qualified Rel8.Query.Exists as Q (absent, exists, present)
...
@unionWith (liftA2 (<>))@.
-}
instance (EqTable k, Table Expr a, Semigroup a) => Semigroup (Tabulation k a) where
  (<>) = alignWith (theseTable id id (<>))


instance (EqTable k, Table Expr a, Semigroup a) => Monoid (Tabulation k a) where
  mempty = emptyTable
...
materialize ::
  (Table Expr k, Table Expr a) =>
  Tabulation k a ->
  (Tabulation k a -> Query b) ->
  Query b
