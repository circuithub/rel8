   ...
   instance Rel8able $(TH.conT name) where
          ...
          gfromColumns $( TH.varP contextName ) $(columnsP)=
            case $( TH.varE contextName ) of
              SExpr      -> $(constructorE id (\_ x -> TH.VarE 'gfromColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))
              SField     -> $(constructorE id (\_ x -> TH.VarE 'gfromColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))
              SName      -> $(constructorE id (\_ x -> TH.VarE 'gfromColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))
              SResult    -> $(constructorE (\x -> TH.VarE 'runIdentity `TH.AppE` x) (\_ x -> TH.VarE 'gfromColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))

          gtoColumns $(TH.varP contextName) $( constructorP ) =
            case $( TH.varE contextName ) of
              SExpr      -> $(columnsE id (\_ x -> TH.VarE 'gtoColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))
              SField     -> $(columnsE id (\_ x -> TH.VarE 'gtoColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))
              SName      -> $(columnsE id (\_ x -> TH.VarE 'gtoColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))
              SResult    -> $(columnsE (\x -> TH.ConE 'Identity `TH.AppE` x) (\_ x -> TH.VarE 'gtoColumns `TH.AppE` TH.VarE contextName `TH.AppE` x))

          gfromResult $columnsP =
              -- TODO: get rid of type application. Use a signature that references the generic value instead
              $( constructorE (\x -> TH.VarE 'runIdentity `TH.AppE` x) (\ft x -> (TH.VarE 'gfromResult `TH.AppTypeE` ft `TH.AppE` x)))

          gtoResult $constructorP =
              -- TODO: get rid of type application. Use a signature that references the generic value instead
              $( columnsE (\x -> TH.ConE 'Identity `TH.AppE` x) (\ft x -> TH.VarE 'gtoResult `TH.AppTypeE` ft `TH.AppE` x))
   |]
