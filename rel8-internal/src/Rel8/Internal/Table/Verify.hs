...
module Rel8.Internal.Table.Verify
  ( getSchemaErrors
  , SomeTableSchema(..)
  , showCreateTable
...
import Rel8.Internal.Column.List ( HList )
import Rel8.Internal.Expr ( Expr )
import Rel8.Internal.Expr.Eq ((==.))
import Rel8.Internal.Expr.Ord ((>.))
import Rel8.Internal.Expr.Order (asc)
import Rel8.Internal.Generic.Rel8able (GFromExprs, Rel8able)
import Rel8.Internal.Query ( Query )
import Rel8.Internal.Query.Each (each)
import Rel8.Internal.Query.Filter (filter)
import Rel8.Internal.Query.List (many)
import Rel8.Internal.Query.Order (orderBy)
...
import Rel8.Internal.Schema.Spec (Spec (Spec))
import qualified Rel8.Internal.Schema.Spec
import Rel8.Internal.Schema.Table ( TableSchema(..) )
import Rel8.Internal.Statement.Run (run1)
import Rel8.Internal.Statement.Select (select)
import Rel8.Internal.Table (Columns, toColumns)
import Rel8.Internal.Table.List ( ListTable )
import Rel8.Internal.Table.Name (namesFromLabelsWith)
import Rel8.Internal.Table.Rel8able ()
import Rel8.Internal.Table.Serialize (ToExprs, lit)
import Rel8.Internal.Type ( DBType(..) )
import Rel8.Internal.Type.Eq ( DBEq )
import Rel8.Internal.Type.Information (parseTypeInformation)
