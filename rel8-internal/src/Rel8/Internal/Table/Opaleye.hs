...
module Rel8.Internal.Table.Opaleye
  ( attributes
  , binaryspec
  , distinctspec
  , exprs
...
import Rel8.Internal.Schema.Name ( Name( Name ), Selects, ppColumn )
import Rel8.Internal.Schema.QualifiedName (QualifiedName (QualifiedName))
import Rel8.Internal.Schema.Spec ( Spec(..) )
import Rel8.Internal.Schema.Table ( TableSchema(..), ppTable )
import Rel8.Internal.Table ( Table, fromColumns, toColumns )
import Rel8.Internal.Type.Information ( typeName )
import Rel8.Internal.Type.Name (showTypeName)
...
valuesspec = dimap toColumns fromColumns $
  ...
  where
    typeName = showTypeName . Rel8.Internal.Type.Information.typeName . info . hfield hspecs
